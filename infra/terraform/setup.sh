...
if [ "$CONFIRM" = "yes" ]; then
    ...
    if [ $STATUS -eq 0 ]; then
        ...
        echo "--------------------------"
        echo "Since the AI model is large and gitignored, you must upload it manually:"
        echo "Run this command from your LAPTOP terminal:"
        echo "scp -i ai-fake-news-detector-key.pem -r ../../model_training/saved_model/fake-news-bert/* ubuntu@\$(terraform output -raw instance_public_ip):/mnt/data/model/"
        echo ""
